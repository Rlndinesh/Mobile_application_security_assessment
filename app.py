...
def perform_dynamic_analysis(apk_path):
    try:
        ...
        time.sleep(3)  # Allow the app to fully start

        # Ensure Frida server is running before attaching
        subprocess.run(["adb", "shell", "/data/local/tmp/frida-server &"], shell=True)
        time.sleep(2)

        # Attach Frida to the running app using the PID
        ...
        if pid_result.returncode != 0 or not pid_result.stdout.strip():
            return {"error": "Failed to find PID for the app."}

        pid = pid_result.stdout.strip()  # Extract the PID

        # Attach Frida using the PID instead of -s or --no-pause
        frida_result = subprocess.run(
            ["frida", "-U", "-p", pid, "-n", package_name, "-e", "console.log('Dynamic analysis started');"],
            capture_output=True, text=True
        )
